...
class StoreIdForTaxonomies < ActiveRecord::Migration
  def self.up
    if table_exists?('taxonomies')
      ...
      add_column :spree_taxonomies, :store_id, :integer unless column_exists?(:spree_taxonomies, :store_id)
      add_index :spree_taxonomies, :store_id unless index_exists?(:spree_taxonomies, :store_id)
    end
  end
