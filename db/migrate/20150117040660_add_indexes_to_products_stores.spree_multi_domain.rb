...
class AddIndexesToProductsStores < ActiveRecord::Migration
  def self.up
    add_index :products_stores, :product_id unless index_exists?(:products_stores, :product_id)
    add_index :products_stores, :store_id unless index_exists?(:products_stores, :store_id)
  end
